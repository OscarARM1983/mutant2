...
def isMutant():
    try:
        ...
        if evalua_letra == 0:
            ...
            if evaluador == 0: 
                cursor=db.connect()
                sql=sqlalchemy.text("""INSERT INTO dna_verificados (dna0, dna1, dna2, dna3, dna4, dna5) 
                VALUES ('{0}', '{1}', '{2}', '{3}', '{4}', '{5}')""".format(dna[0], dna[1], dna[2], dna[3], dna[4], dna[5]))
                cursor.execute(sql)
                cursor.commit() #Para confirmar la acción de registrar datos
                #return jsonify({'mensaje':"DNA registrado"})          
                """Se agrega a campo mutante 1 si es mutante o 0 si no lo es """
                if total_secuencias > 1:
                    cursor=db.connect()
                    sql2=sqlalchemy.text("SELECT MAX(id) FROM dna_verificados;")
                    cursor.execute(sql2)
                    max_id = cursor.fetchone()
                    cursor=db.connect()
                    sql3=sqlalchemy.text(f"UPDATE dna_verificados SET mutante = 1 WHERE id={max_id[0]};")
                    cursor.execute(sql3)
                    cursor.commit()
                    ...
                    max_id = cursor.fetchone()
                    cursor=db.connect()
                    sql3=sqlalchemy.text(f"UPDATE dna_verificados SET mutante = 0 WHERE id={max_id[0]};")
                    cursor.execute(sql3)
                    cursor.commit()
...
if __name__ == '__main__':
    ...
    app.run(host="127.0.0.1", port=8080, debug=True) # Permite hacer cambios y actualizar automáticamente el servidor